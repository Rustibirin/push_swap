...
The push_swap project serves as an initial exploration of the underlying logic behind algorithms. The objective was to write a program capable of sorting data stored in a stack, using an empty auxiliary stack, a limited set of possible instructions, and optimizing the total number of operations required for the sort. The program uses several sorting algorithms of varying complexity and is capable of selecting the most appropriate one based on the degree of disorder in the initial stack.
